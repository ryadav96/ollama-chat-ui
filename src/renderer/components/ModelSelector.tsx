...
const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  const currentModel = models.find(model => model.name === selectedModel)?.name || 'Select a model'
      ...
      <div className="p-4 border-b">
        ...
        <div className="relative mb-2">
          <button
            ...
            className="w-full flex items-center justify-between p-2 bg-background border rounded-md hover-shadow-effect text-sm"
          ...
          {isDropdownOpen && models.length > 0 && (
            <div className="absolute z-50 mt-1 w-full bg-popover rounded-md shadow-lg ring-1 ring-black ring-opacity-5 max-h-60 overflow-y-auto">
              <div className="py-1">
                {models.map((model) => (
                  <div
                    key={model.name}
